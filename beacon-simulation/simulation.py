...
BEACON_1_POSITION = np.array([0, 0.75, 0.35])
BEACON_2_POSITION = np.array([0, -0.75, 0.35])
BEACON_3_POSITION = np.array([-1, 0, 0.35])
...
def lemniscate_of_bernoulli(t):
    scale = 2.5 / (3 - math.cos(2 * t))
    x = scale * math.cos(t)
    y = scale * math.sin(2 * t) / 1.5

    return np.array([x, y])


def generate_full_trajectory():
    traj_t = np.linspace(0, SIMULATION_DURATION,
                         SIMULATION_DURATION * SIMULATION_UPDATE_RATE)
    traj_x = []
    traj_y = []
    for i in traj_t:
...
def plot_errors(traj_x, traj_y, ekf_x, ekf_y):
    ...
    fig_x.set_title('mean {:.6f}, dev {:.6f}'.format(
        np.mean(err_x), np.std(err_x)))
    fig_y.set_title('mean {:.6f}, dev {:.6f}'.format(
        np.mean(err_y), np.std(err_y)))
...
def main():
    ...
    for i in range(int(SIMULATION_DURATION * SIMULATION_UPDATE_RATE)):
        ...
        kalman.predict(0)
        kalman.measure(np.array([d1, d2, d3]).reshape(
            [3, 1]), MEASUREMENT_COVARIANCE_MATRIX)
        ...
        plot_position(fig, BEACON_1_POSITION[0:2], 'black')
        plot_position(fig, BEACON_2_POSITION[0:2], 'black')
        plot_position(fig, BEACON_3_POSITION[0:2], 'black')
